...
const TIER_CARDS = [
  {
    ...
    color: "#1E88E5",
    icon: "🗺️",
    desc: "Get a real 7/30/90-day business roadmap in minutes — no credit card, no fluff.",
    href: "/starter",
  },
  {
    name: "Pro — $249/mo",
    color: "#D4A017",
    icon: "⚡",
    desc: "Full blueprint, credit & funding readiness, sales scripts, PDF export, and branding support.",
    href: "/pro",
  },
  {
    name: "Elite — $499/mo",
    color: "#00C9B1",
    icon: "🧠",
    desc: "Advanced strategist guidance, financial projections, vendor & funding resource center.",
    href: "/elite",
  },
  {
    ...
    color: "#d4af37",
    icon: "🏆",
    desc: "One payment. Full lifetime access. Every feature. Only 200 spots — ever.",
    href: "/founders",
  },
];
      ...
      <div className="mx-auto max-w-6xl px-5 py-16 md:py-24">
        <div className="grid gap-12 md:grid-cols-2 md:items-start lg:gap-16">
          ...
          <div className="order-2 md:order-1">
            {/* Brand headline */}
            <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-[#1A2235] bg-[#0F1520] px-4 py-1.5 text-xs font-bold uppercase tracking-widest" style={{ color: "#D4A017" }}>
            ...
            <p className="mt-4 text-base leading-7 text-slate-400">
              PEN2PRO RMIE — Rapid Monetization Intelligence Engine — turns your idea, skills, or business concept into a real roadmap with monetization strategy, credit readiness, and execution support.
            </p>

            {/* Tier benefits */}
            <div className="mt-8 space-y-3">
              <p className="text-xs font-bold uppercase tracking-widest text-slate-500">What You Get With Each Tier</p>
              {TIER_CARDS.map((tier) => (
                <Link
                  key={tier.name}
                  to={tier.href}
                  className="flex items-start gap-4 rounded-xl border border-[#1A2235] bg-[#0F1520] p-4 transition hover:border-[#1A2D50] hover:bg-[#111827]"
                >
                  <div
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-xs leading-relaxed text-slate-400">{tier.desc}</p>
                  </div>
                  <span className="ml-auto mt-1 shrink-0 text-slate-600 text-xs">→</span>
                </Link>
              ))}
            </div>
          ...
          <div className="order-1 md:order-2">
              ...
              {tab === "login" && (
                <form onSubmit={handleLogin} className="space-y-5">
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full rounded-xl py-3 text-sm font-black text-[#080C14] disabled:opacity-60 transition"
                    style={{ background: "linear-gradient(135deg, #D4A017, #F0C040)" }}
                  >
            ...
            <div className="mt-5 text-center">
              <p className="text-sm text-slate-500">
                Want to try first?{" "}
                <Link to="/starter" className="font-bold hover:opacity-80 transition" style={{ color: "#1E88E5" }}>
