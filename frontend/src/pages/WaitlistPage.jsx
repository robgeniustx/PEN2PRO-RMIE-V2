...
export default function WaitlistPage() {
  ...
  // countdown removed — platform is now live
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (res.status === 409) {
          setErrorMsg("That email is already on the list. You're already in — check your inbox for updates!");
          setStatus("error");
        } else {
        ...
        <div className="flex min-h-[80vh] items-center justify-center px-5 py-16">
          <div className="mx-auto w-full max-w-lg text-center">
            ...
            <p className="text-base leading-7 text-slate-400 mb-8">
              <strong className="text-white">{form.name || "Welcome"}</strong>, you're officially on the PEN2PRO
              list.{" "}
              <strong style={{ color: "#D4A017" }}>The platform is live — you can start right now.</strong>
              {refParam && (
            ...
            <div className="rounded-2xl border border-[#1A2235] bg-[#0F1520] p-6 mb-8 text-left">
              ...
              <ul className="space-y-3">
                {[
                  "Confirmation details are saved — check your inbox for the welcome email",
                  "Founding members get locked-in pricing forever — upgrade anytime",
                  "Start your free roadmap right now — the platform is live",
                  "Share your referral link to earn affiliate commissions on every upgrade",
                ].map((item) => (
        ...
        <div className="mx-auto max-w-5xl grid gap-12 md:grid-cols-2 md:items-start">
          ...
          <div>
            ...
            <h1 className="font-display text-4xl font-black leading-tight text-white md:text-5xl">
              ...
              <span className="gradient-text">Shape What's Next.</span>
            </h1>

            <p className="mt-5 text-base leading-7 text-slate-400">
              PEN2PRO is <strong className="text-white">now live</strong>. Founding members lock in their pricing
              for life, get first access to every new feature, and receive done-with-you onboarding support
              that won't be available after founding spots are gone.
            </p>

            {/* Live Status */}
            <div className="mt-8 inline-flex items-center gap-3 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-5 py-3">
              <span className="h-2.5 w-2.5 rounded-full bg-emerald-400 animate-pulse shrink-0" />
              <span className="text-sm font-bold text-emerald-300">Platform is Live — Start Building Now</span>
            </div>
            ...
            <div className="rounded-2xl border border-[#1A2235] bg-[#0F1520] p-7">
              ...
              <form onSubmit={submit} className="space-y-4">
                ...
                <button type="submit" disabled={status === "loading"}
                  ...
                  {status === "loading" ? "Securing Your Spot..." : "🔒 Secure My Spot — Get Early Access"}
                </button>
              </form>
