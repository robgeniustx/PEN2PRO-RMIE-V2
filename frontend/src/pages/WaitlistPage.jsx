...
function useCountdown(target) {
  const [t, setT] = useState({ live: false, d: 0, h: 0, m: 0, s: 0 });
  useEffect(() => {
    const calc = () => {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (res.status === 409) {
          setErrorMsg("That email is already on the list. You're already in — we'll see you at full launch on August 1!");
          setStatus("error");
        } else {
        ...
        <div className="flex min-h-[80vh] items-center justify-center px-5 py-16">
          <div className="mx-auto w-full max-w-lg text-center">
            ...
            <p className="text-base leading-7 text-slate-400 mb-8">
              ...
              PEN2PRO full platform launch on{" "}
            ...
            <div className="rounded-2xl border border-[#1A2235] bg-[#0F1520] p-6 mb-8 text-left">
              ...
              <ul className="space-y-3">
                {[
                  "Confirmation details are saved — watch for our August 1 launch email",
        ...
        <div className="mx-auto max-w-5xl grid gap-12 md:grid-cols-2 md:items-start">
          ...
          <div>
            ...
            <h1 className="font-display text-4xl font-black leading-tight text-white md:text-5xl">
              Secure Your Spot.<br />
              <span className="gradient-text">Full Launch Aug 1.</span>
            </h1>

            <p className="mt-5 text-base leading-7 text-slate-400">
              PEN2PRO full platform launch is <strong className="text-white">August 1, 2026</strong>. Founding members
            ...
            <div className="mt-8">
              {cd.live ? (
                <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/40 bg-emerald-500/10 px-4 py-2">
                  <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
                  <span className="text-sm font-black uppercase tracking-widest text-emerald-400">Now Open — Join Today</span>
                </div>
              ) : (
                <>
