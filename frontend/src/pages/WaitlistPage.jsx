  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (res.status === 409) {
          setErrorMsg("That email is already on the list. You're already secured — we'll be in touch with early access details!");
          setStatus("error");
        } else {
        ...
        <div className="flex min-h-[80vh] items-center justify-center px-5 py-16">
          <div className="mx-auto w-full max-w-lg text-center">
            ...
            <p className="text-base leading-7 text-slate-400 mb-8">
              <strong className="text-white">{form.name || "Welcome"}</strong>, your spot is secured on
              the PEN2PRO platform.
              {refParam && (
            ...
            <div className="rounded-2xl border border-[#1A2235] bg-[#0F1520] p-6 mb-8 text-left">
              ...
              <ul className="space-y-3">
                {[
                  "Confirmation saved — early access details are on the way",
        ...
        <div className="mx-auto max-w-5xl grid gap-12 md:grid-cols-2 md:items-start">
          ...
          <div>
            ...
            <p className="mt-5 text-base leading-7 text-slate-400">
              ...
              lock in their pricing for life, get first access to every new feature, and receive
              done-with-you launch support only available at this tier.
            </p>

            {/* Now Live badge */}
            <div className="mt-8 inline-flex items-center gap-3 rounded-xl border px-5 py-3"
              style={{ borderColor: "rgba(0,201,177,0.3)", background: "rgba(0,201,177,0.08)" }}>
              <span className="h-2.5 w-2.5 rounded-full bg-emerald-400 animate-pulse shrink-0" />
              <span className="text-sm font-bold text-teal-300">Platform is Live — Founding Spots Still Available</span>
            </div>
