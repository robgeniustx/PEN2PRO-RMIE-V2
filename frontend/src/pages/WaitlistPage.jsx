  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (res.status === 409) {
          setErrorMsg("That email is already on the list. You're already in — check your email for access details.");
          setStatus("error");
        } else {
        ...
        <div className="flex min-h-[80vh] items-center justify-center px-5 py-16">
          <div className="mx-auto w-full max-w-lg text-center">
            ...
            <p className="text-base leading-7 text-slate-400 mb-8">
              <strong className="text-white">{form.name || "Welcome"}</strong>, your founding spot is secured. You now have early access to PEN2PRO.
              {refParam && (
            ...
            <div className="rounded-2xl border border-[#1A2235] bg-[#0F1520] p-6 mb-8 text-left">
              ...
              <ul className="space-y-3">
                {[
                  "Confirmation details are saved — watch for our welcome email",
                  "Founding members lock in pricing forever — never increases",
        ...
        <div className="mx-auto max-w-5xl grid gap-12 md:grid-cols-2 md:items-start">
          ...
          <div>
            ...
            <h1 className="font-display text-4xl font-black leading-tight text-white md:text-5xl">
              Secure Your Spot.<br />
              <span className="gradient-text">Now Live.</span>
            </h1>

            <p className="mt-5 text-base leading-7 text-slate-400">
              PEN2PRO is now live in early access. Founding members
              lock in their pricing for life, get first access to every new feature, and receive done-with-you support
              that won't be available to future subscribers.
            </p>

            {/* Live Badge */}
            <div className="mt-8 flex items-center gap-3 rounded-xl border border-emerald-900/40 bg-emerald-950/20 px-5 py-3">
              <span className="h-2.5 w-2.5 rounded-full bg-emerald-400 animate-pulse shrink-0" />
              <span className="text-sm font-bold text-emerald-400">Platform live — founding spots available now</span>
            </div>
            ...
            <div className="rounded-2xl border border-[#1A2235] bg-[#0F1520] p-7">
              ...
              <form onSubmit={submit} className="space-y-4">
                ...
                <button type="submit" disabled={status === "loading"}
                  ...
                  {status === "loading" ? "Securing Your Spot..." : "🔒 Claim My Founding Spot"}
                </button>
              </form>
