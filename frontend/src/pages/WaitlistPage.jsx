  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (res.status === 409) {
          setErrorMsg("That email is already on the list. You're already in — check your inbox for access details!");
          setStatus("error");
        } else {
        ...
        <div className="flex min-h-[80vh] items-center justify-center px-5 py-16">
          <div className="mx-auto w-full max-w-lg text-center">
            ...
            <p className="text-base leading-7 text-slate-400 mb-8">
              <strong className="text-white">{form.name || "Welcome"}</strong>, you're on the PEN2PRO priority list. The platform is now live — start your free roadmap below or wait for your personalized onboarding email.
              {refParam && (
            ...
            <div className="rounded-2xl border border-[#1A2235] bg-[#0F1520] p-6 mb-8 text-left">
              ...
              <ul className="space-y-3">
                {[
                  "You're confirmed — check your inbox for platform access details",
                  "Founding members lock in pricing for life — never increases",
                  "Start your free roadmap right now and build your blueprint",
          ...
          <div>
            ...
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border px-4 py-2"
              ...
              <span className="text-xs font-bold uppercase tracking-widest" style={{ color: "#D4A017" }}>
                Platform Live — Limited Founders Lifetime Spots
              </span>
            </div>

            <h1 className="font-display text-4xl font-black leading-tight text-white md:text-5xl">
              Secure Your Spot.<br />
              <span className="gradient-text">Platform Is Live.</span>
            </h1>

            <p className="mt-5 text-base leading-7 text-slate-400">
              PEN2PRO is <strong className="text-white">now live</strong>. Founding members lock in their pricing for life, get first access to every feature, and receive done-with-you launch support that won't be available after founding spots close.
            </p>

            {/* Live Status */}
            <div className="mt-8 flex items-center gap-3 rounded-2xl border border-emerald-500/30 bg-emerald-950/20 px-5 py-3">
              <span className="h-2.5 w-2.5 rounded-full bg-emerald-400 animate-pulse shrink-0" />
              <span className="text-sm font-bold text-emerald-400">Platform is live — start your free roadmap now or secure a Founders Lifetime spot</span>
            </div>
            ...
            <div className="rounded-2xl border border-[#1A2235] bg-[#0F1520] p-7">
              ...
              <form onSubmit={submit} className="space-y-4">
                ...
                <button type="submit" disabled={status === "loading"}
                  ...
                  {status === "loading" ? "Securing Your Spot..." : "🔒 Secure My Founders Spot"}
                </button>
              </form>
