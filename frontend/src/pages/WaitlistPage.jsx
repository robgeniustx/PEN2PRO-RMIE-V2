  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (res.status === 409) {
          setErrorMsg("That email is already on the list. You're already in — welcome to PEN2PRO!");
          setStatus("error");
        } else {
        ...
        <div className="flex min-h-[80vh] items-center justify-center px-5 py-16">
          <div className="mx-auto w-full max-w-lg text-center">
            ...
            <p className="text-base leading-7 text-slate-400 mb-8">
              <strong className="text-white">{form.name || "Welcome"}</strong>, you're now part of the PEN2PRO community.
              Founding members get{" "}
              <strong style={{ color: "#D4A017" }}>locked-in pricing for life</strong> and first access to every new feature.
              {refParam && (
            ...
            <div className="rounded-2xl border border-[#1A2235] bg-[#0F1520] p-6 mb-8 text-left">
              ...
              <ul className="space-y-3">
                {[
                  ...
                  "Start your free roadmap right now — it's live today",
        ...
        <div className="mx-auto max-w-5xl grid gap-12 md:grid-cols-2 md:items-start">
          ...
          <div>
            ...
            <h1 className="font-display text-4xl font-black leading-tight text-white md:text-5xl">
              PEN2PRO Is Live.<br />
              <span className="gradient-text">Secure Founders Pricing.</span>
            </h1>

            <p className="mt-5 text-base leading-7 text-slate-400">
              PEN2PRO is now open. Founding members lock in their pricing for life, get first access to every
              new feature, and receive done-with-you onboarding support — but{" "}
              <strong className="text-white">only 200 founder spots are available</strong>.
            </p>

            {/* Live badge */}
            <div className="mt-8 inline-flex items-center gap-3 rounded-2xl border border-emerald-500/30 bg-emerald-900/20 px-5 py-3">
              <span className="h-3 w-3 rounded-full bg-emerald-400 animate-pulse" />
              <span className="text-sm font-bold text-emerald-400">Platform is live — join today and start building</span>
            </div>
            ...
            <div className="rounded-2xl border border-[#1A2235] bg-[#0F1520] p-7">
              ...
              <form onSubmit={submit} className="space-y-4">
                ...
                <button type="submit" disabled={status === "loading"}
                  ...
                  {status === "loading" ? "Securing Your Spot..." : "🔒 Claim My Founders Spot"}
                </button>
              </form>
