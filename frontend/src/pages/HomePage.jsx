...
function useCountdown(target) {
  ...
  useEffect(() => {
    const calc = () => {
      ...
      if (diff <= 0) return setT({ live: true, d: 0, h: 0, m: 0, s: 0 });
      setT({ live: false, d: Math.floor(diff / 86400000), h: Math.floor((diff % 86400000) / 3600000), m: Math.floor((diff % 3600000) / 60000), s: Math.floor((diff % 60000) / 1000) });
    };
    calc();
    ...
    <div className="min-h-screen bg-[#080C14]">
      ...
      <section className="relative overflow-hidden px-5 pt-20 pb-28 md:pt-28 md:pb-36 text-center">
        ...
        <div className="relative mx-auto max-w-4xl">
          <div className="animate-fade-up mb-6 inline-flex items-center gap-2 rounded-full border px-4 py-2" style={{ borderColor: 'rgba(212,160,23,0.3)', background: 'rgba(212,160,23,0.08)' }}>
            ...
            <span className="text-xs font-bold uppercase tracking-widest" style={{ color: '#D4A017' }}>Now Open — Full Launch August 1, 2026</span>
          </div>
          ...
          <div className="mt-14 flex flex-col items-center">
            {cd.live ? (
              <div className="flex flex-col items-center gap-3">
                <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/40 bg-emerald-500/10 px-5 py-2">
                  <span className="h-2.5 w-2.5 rounded-full bg-emerald-400 animate-pulse" />
                  <span className="text-sm font-black uppercase tracking-widest text-emerald-400">Platform Is Live — Waitlist Open</span>
                </div>
                <p className="text-xs text-slate-600">Founders pricing locks in when you join the waitlist today</p>
              </div>
            ) : (
              <>
                <p className="mb-4 text-xs font-bold uppercase tracking-widest text-slate-500">Full Launch Countdown — August 1, 2026</p>
      ...
      <section className="border-y border-[#1A2235] bg-[#0F1520] px-5 py-8">
        <div className="mx-auto max-w-5xl grid grid-cols-2 gap-6 md:grid-cols-4 text-center">
          {[["2,847+","Roadmaps Generated"],["$0","Required to Start"],["7 Days","To First Plan"],["Aug 1","Full Launch Date"]].map(([v,l]) => (
            <div key={l}>
