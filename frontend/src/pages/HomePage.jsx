    ...
    <div className="min-h-screen bg-[#080C14]">
      ...
      <section className="relative overflow-hidden px-5 pt-20 pb-28 md:pt-28 md:pb-36 text-center">
        ...
        <div className="relative mx-auto max-w-4xl">
          <div className="animate-fade-up mb-6 inline-flex items-center gap-2 rounded-full border px-4 py-2" style={{ borderColor: 'rgba(212,160,23,0.3)', background: 'rgba(212,160,23,0.08)' }}>
            ...
            <span className="text-xs font-bold uppercase tracking-widest" style={{ color: '#D4A017' }}>Now Live — Free Roadmap Available</span>
          </div>
          ...
          <div className="mt-14 flex flex-col items-center">
            <p className="mb-4 text-xs font-bold uppercase tracking-widest text-slate-500">Platform Now Live — Founders Pricing Still Available</p>
            <div className="flex flex-wrap justify-center gap-6">
              {[
                { label: "Free to Start", icon: "$0" },
                { label: "Full Roadmap in", icon: "5 min" },
                { label: "Founders Spots Left", icon: "Limited" },
              ].map(({ label, icon }) => (
                <div key={label} className="flex flex-col items-center rounded-xl border border-[#1A2235] bg-[#0F1520] px-6 py-4 min-w-[110px]">
                  <span className="font-display text-2xl font-black" style={{ color: '#D4A017' }}>{icon}</span>
                  <span className="mt-1 text-[10px] font-bold uppercase tracking-widest text-slate-500">{label}</span>
                </div>
              ))}
            </div>
            <p className="mt-3 text-xs text-slate-600">Founders lifetime pricing locks in when you sign up today</p>
          </div>
        </div>
      </section>
      ...
      <section className="border-y border-[#1A2235] bg-[#0F1520] px-5 py-8">
        <div className="mx-auto max-w-5xl grid grid-cols-2 gap-6 md:grid-cols-4 text-center">
          {[["2,847+","Roadmaps Generated"],["$0","Required to Start"],["5 min","To First Plan"],["Now Live","Platform Status"]].map(([v,l]) => (
            <div key={l}>
