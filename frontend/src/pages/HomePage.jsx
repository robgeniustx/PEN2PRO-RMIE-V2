    ...
    <div className="min-h-screen bg-[#080C14]">
      ...
      <section className="relative overflow-hidden px-5 pt-20 pb-28 md:pt-28 md:pb-36 text-center">
        ...
        <div className="relative mx-auto max-w-4xl">
          <div className="animate-fade-up mb-6 inline-flex items-center gap-2 rounded-full border px-4 py-2" style={{ borderColor: 'rgba(52,211,153,0.3)', background: 'rgba(52,211,153,0.07)' }}>
            <span className="h-2 w-2 rounded-full bg-emerald-400 animate-pulse" />
            <span className="text-xs font-bold uppercase tracking-widest text-emerald-400">Now Live — Founding Spots Still Available</span>
          </div>
          ...
          <div className="mt-14 flex flex-col items-center">
            <div className="flex items-center gap-8 rounded-2xl border border-[#1A2235] bg-[#0F1520] px-8 py-5">
              {[["200", "Founding Spots Total"], ["136+", "Already Claimed"], ["$1,899", "Founders Price — For Life"]].map(([v, l]) => (
                <div key={l} className="text-center">
                  <p className="font-display text-2xl font-black" style={{ color: '#D4A017' }}>{v}</p>
                  <p className="mt-1 text-[10px] font-bold uppercase tracking-widest text-slate-500">{l}</p>
                </div>
              ))}
            </div>
            <p className="mt-3 text-xs text-slate-600">Founders pricing locks in when you claim your spot today</p>
          </div>
        </div>
      </section>
