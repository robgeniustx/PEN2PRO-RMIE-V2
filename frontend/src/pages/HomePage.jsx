    ...
    <div className="min-h-screen bg-[#080C14]">
      ...
      <section className="relative overflow-hidden px-5 pt-20 pb-28 md:pt-28 md:pb-36 text-center">
        ...
        <div className="relative mx-auto max-w-4xl">
          <div className="animate-fade-up mb-6 inline-flex items-center gap-2 rounded-full border px-4 py-2" style={{ borderColor: 'rgba(212,160,23,0.3)', background: 'rgba(212,160,23,0.08)' }}>
            ...
            <span className="text-xs font-bold uppercase tracking-widest" style={{ color: '#D4A017' }}>Now Live — Free Roadmap Available Today</span>
          </div>
          ...
          <div className="mt-14 flex flex-col items-center gap-3">
            <div className="flex items-center gap-6 rounded-2xl border border-[#1A2235] bg-[#0F1520] px-8 py-4">
              {[["Free","Start a roadmap in minutes"],["Pro","Full execution strategy"],["Elite","Done-with-you guidance"],["Founders","Lifetime access — limited"]].map(([label, sub]) => (
                <div key={label} className="hidden sm:flex flex-col items-center text-center">
                  <span className="text-sm font-black text-white">{label}</span>
                  <span className="text-[10px] text-slate-500 mt-0.5 whitespace-nowrap">{sub}</span>
                </div>
              ))}
              <div className="flex sm:hidden flex-col items-center text-center">
                <span className="text-sm font-black" style={{ color: '#D4A017' }}>4 Tiers Available</span>
                <span className="text-[10px] text-slate-500 mt-0.5">Free · Pro · Elite · Founders</span>
              </div>
            </div>
            <p className="text-xs text-slate-600">Founders lifetime pricing still available — limited spots remaining</p>
          </div>
        </div>
      </section>
      ...
      <section className="border-y border-[#1A2235] bg-[#0F1520] px-5 py-8">
        <div className="mx-auto max-w-5xl grid grid-cols-2 gap-6 md:grid-cols-4 text-center">
          {[["2,847+","Roadmaps Generated"],["$0","Required to Start"],["7 Days","To First Plan"],["Now Live","Platform Status"]].map(([v,l]) => (
            <div key={l}>
