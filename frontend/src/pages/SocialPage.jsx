...
export default function SocialPage() {
  ...
  const [tier, setTier] = useState(params.get('tier') || 'free');
  ...
  return (
    <div className="p-4 bg-slate-950 min-h-screen">
      <button className="bg-orange-500 px-3 py-2 rounded" onClick={submit}>
        Generate Social Engine
      </button>
      <Link className="block mt-4 text-orange-300 underline" to={`/leads?tier=${tier}`}>
        Save Interested People as Leads
      </Link>
      <pre className="text-white text-xs mt-4 whitespace-pre-wrap">
