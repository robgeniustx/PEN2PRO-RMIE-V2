...
const PLANS_DROPDOWN = [
  { label: "Free Roadmap",    path: "/starter",      color: "#00BFA5" },
  { label: "Pro",             path: "/pro",           color: "#1E88E5" },
  { label: "Elite",           path: "/elite",         color: "#FF8A00" },
  { label: "Legacy Founder",  path: "/founders",      color: "#9B59B6" },
];
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3.5">
        ...
        <div className="hidden items-center gap-5 xl:flex">
          ...
          <div className="relative" ref={plansRef}>
            ...
            >
              Plans
              <svg className={`h-3.5 w-3.5 transition-transform ${plansOpen ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            {plansOpen && (
              <div className="absolute right-0 top-full mt-2 w-48 rounded-xl border border-[#1A2D50] bg-[#0D1520] py-2 shadow-2xl">
                {PLANS_DROPDOWN.map((p) => (
                  <Link
                    key={p.path}
                    to={p.path}
                    onClick={() => setPlansOpen(false)}
                    className="flex items-center gap-2.5 px-4 py-2.5 text-sm font-semibold text-slate-300 transition hover:bg-[#1A2235] hover:text-white"
                  >
