      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3.5">
        ...
        <div className="hidden items-center gap-5 xl:flex">
          ...
          <div className="relative" ref={plansRef}>
            ...
            >
              Plans
              <span className={`text-xs transition-transform ${plansOpen ? "rotate-180" : ""}`}>▾</span>
            </button>
            {plansOpen && (
              <div className="absolute right-0 top-full mt-2 w-44 rounded-xl border border-[#1A2D50] bg-[#0F1520] py-2 shadow-xl z-50">
                {PLANS_DROPDOWN.map((d) => (
                  <Link
                    key={d.path}
                    to={d.path}
                    onClick={() => setPlansOpen(false)}
                    className={`block px-4 py-2.5 text-sm font-semibold transition-colors ${
                      isActive(d.path) ? "text-[#FF8A00]" : "text-slate-300 hover:text-white hover:bg-[#1A2235]"
                    }`}
                  >
                    {d.label}
                  </Link>
