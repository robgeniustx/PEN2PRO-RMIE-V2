      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-3.5">
        ...
        <div className="hidden items-center gap-5 xl:flex">
          ...
          <div className="relative" ref={plansRef}>
            ...
            {plansOpen && (
              <div className="absolute right-0 top-full mt-2 w-52 rounded-2xl border border-[#1A2D50] bg-[#0F1520] py-2 shadow-2xl z-50">
                {PLANS_DROPDOWN.map((p) => (
                  <Link key={p.path} to={p.path} onClick={() => setPlansOpen(false)}
                    className="block px-4 py-2.5 text-sm font-semibold text-slate-300 hover:bg-[#1A2235] hover:text-white transition-colors">
                    {p.label}
                  </Link>
                ))}
              </div>
