...
def has_tier(required_tier: str, user_tier: str) -> bool:
    """Return True if user_tier meets or exceeds required_tier in the tier hierarchy."""
    return TIER_RANK.get(normalize_tier(user_tier), 0) >= TIER_RANK.get(normalize_tier(required_tier), 0)
