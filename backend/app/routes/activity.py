...
def log(payload: ActivityLogCreate, db: Session = Depends(get_db)): s=AutomationService(db); l=s.log_activity(payload.action_type,payload.description,payload.extra_data); s.db.commit(); s.db.refresh(l); return l
